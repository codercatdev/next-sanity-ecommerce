...
async function syncProductToSanity(product: Stripe.Product) {
  ...
  const imageRefs = await Promise.all(product.images.map(async (imageUrl) => {
    const asset = await sanityClient.fetch(`*[_type == 'sanity.imageAsset' && url == $imageUrl][0]`, { imageUrl });
    if (asset) {
      return {
        _key: asset.assetId,
        _type: 'image',
  ...
  if (typeof product.default_price === 'string') {
    const price = await sanityClient.fetch(`*[_type == 'price' && stripePriceId == $priceId][0]`, { priceId: product.default_price });
    if (price) {
      defaultPriceRef = {
        _type: 'reference',
        _ref: price._id,
...
async function deleteProductFromSanity(productId: string) {
  ...
  await sanityClient.delete({ query: `*[_type == "product" && stripeProductId == $productId]`, params: { productId } })
  revalidateTag('products')
...
async function syncPriceToSanity(price: Stripe.Price) {
  ...
  const productDoc = await sanityClient.fetch('*[_type == "product" && stripeProductId == $productId][0]', {
  ...
  await sanityClient.createOrReplace(priceDoc)
...
async function deletePriceFromSanity(price: Stripe.Price) {
  ...
  await sanityClient.delete(priceId);
